...
def find_resources_in_state(state_data, modules):
    ...
    for resource in state_data.get("resources", []):
        ...
        for index, instance in enumerate(resource.get("instances", [])):
            ...
            full_name = f"{resource_namespace}{resource['type']}.{resource_name}"

            if index_key:  # Add index key if it exists
                full_name = f'{full_name}["{index_key}"]'
            elif len(resource.get("instances", [])) > 1:  # Add index when multiple instances exist
                full_name = f"{full_name}[{index}]"
            ...
            # Special case for aws_security_group_rule to format id properly
                ...
                ipv6_cidr_blocks = attributes.get("ipv6_cidr_blocks", [])
                prefix_list_ids = attributes.get("prefix_list_ids", [])
                description = attributes.get("description", "")
                self_value = "true" if attributes.get("self", False) else "false"
                source_security_group_id = attributes.get("source_security_group_id", "")

                # Construct ID format
                cidr_blocks_str = ",".join(cidr_blocks) if cidr_blocks else "_"
                ipv6_cidr_blocks_str = ",".join(ipv6_cidr_blocks) if ipv6_cidr_blocks else "_"
                prefix_list_ids_str = ",".join(prefix_list_ids) if prefix_list_ids else "_"
                source_security_group_id_str = source_security_group_id or "_"
                description_str = description or "_"

                resource_id = (
                    f"{security_group_id}_{rule_type}_{protocol}_{from_port}_{to_port}_"
                    f"{cidr_blocks_str}_{ipv6_cidr_blocks_str}_{prefix_list_ids_str}_"
                    f"{description_str}_{self_value}_{source_security_group_id_str}"
                )
