...
def find_resources_in_state(state_data, modules):
    ...
    for resource in state_data.get("resources", []):
        ...
        # Process only resources belonging to the listed modules
        if not any(resource_namespace.startswith(m) for m in modules):
            continue
        ...
        for index, instance in enumerate(resource.get("instances", [])):
            ...
            # Special case for aws_security_group_rule to format id properly
                ...
                if cidr_blocks:
                    ...
                    resource_id = f"{security_group_id}_{rule_type}_{protocol}_{from_port}_{to_port}_{source_security_group_id}"
